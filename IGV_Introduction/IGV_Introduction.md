...
- Load the encode RNA-Seq BAM files: Go to File > Load from ENCODE > Select C2C12 RNA-Seq BAM files (See image in the Pdf) (add image)
...
- You can change the track properties by right-click on the track. Select View as pairs to see the left and right pairs of paired-end reads. (add image)



- You can make a sashimi plot to see reads aligning to each junction. Right-click > Sashimi plot. (add image)
...
- Go to File > Load from ENCODE > Select C2C12 ChIP-Seq bigwig files (add image)



- Reset the track heights to make them comparable. Right click on the track > Set data range. Then change the scale. (add image)
