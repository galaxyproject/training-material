...
for slides in $changed_slides; do
	...
	# Process the slides
	echo $built_slides
	# try move docker output from non-existant /slides/_site...
	docker run --rm --network host -v $(pwd):/slides astefanutti/decktape  automatic -s 1920x1080 http://127.0.0.1:9876/training-material/$slides _site/training-material/$pdf
