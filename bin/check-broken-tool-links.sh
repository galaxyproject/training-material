...
grep -n '\{% tool \[' -R topics | sed 's/:\([0-9]\+\):.*{% tool/\t\1/g;s/)\s*%}.*//g;s/ \[/\t/g;s/](/\t/' | \
	...
	awk -F'\t' '{print "::warning file="$1",line="$2",col=0::Broken tool link, please see the [documentation](https://training.galaxyproject.org/training-material/topics/contributing/tutorials/create-new-tutorial-content/tutorial.html#tool-links)."}' | tee $output
