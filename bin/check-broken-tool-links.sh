...
grep -n '\{% tool \[' -R topics | sed 's/:\([0-9]\+\):.*{% tool/\t\1/g;s/)\s*%}.*//g;s/ \[/\t/g;s/](/\t/' | \
	...
	egrep -i -v '(intermine|Cut1|Filter1|Grep1|Grouping1|Paste1|Remove\+beginning1|Show\+beginning1|Summary_Statistics1|addValue|cat1|comp1|gene2exon1|join1|random_lines1|ucsc_table_direct1|upload1|sort1|wig_to_bigWig|ChangeCaseCount1)' | \
    egrep -v '__[A-Z_]+__' | \
