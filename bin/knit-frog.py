...
cmdhandle.write('export SSL_ROLE=usegalaxy_eu.certbot\n')
cmdhandle.write('ansible-galaxy install -p roles galaxyproject.self-signed-certs\n')
    ...
    if 'data-commit' in diff[-1]:
        ...
        for line in diff[0:-2]:
            ...
            if 'ansible-playbook' in line:
                ...
                cmdhandle.write(line.strip() + " -i ~/.hosts --vault-password-file ~/.vault-password.txt -e \"nginx_ssl_role=${SSL_ROLE} openssl_domains={{ certbot_domains }}\" \n")
