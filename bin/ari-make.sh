...
for slides in $changed_slides; do
	...
	# Process the slides
	echo $built_slides
	docker run --rm --network host -v $(pwd):/slides amouat/decktape  automatic -s 1920x1080 http://127.0.0.1:9876/training-material/$slides /slides/_site/training-material/$pdf
