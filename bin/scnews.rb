...
def filterTutorials(x)
  x =~ %r{topics/single-cell/tutorials/.*/tutorial.*\.md}
end

def filterSlides(x)
  x =~ %r{topics/single-cell/tutorials/.*/slides.*\.html}
end
...
def onlyEnabled(x)
  ...
  topic = x.split('/')[1]
  topic_meta = YAML.load_file("metadata/#{single-cell}.yaml")
...
if newsworthy
  if options[:postToMatrix]
    homeserver = if options[:useTestRoom]
                   ...
                   'https://matrix.org/_matrix/client/r0/rooms/!yuLoaCWKpFHkWPmVEO:gitter.im/send/m.room.message'
                 end
