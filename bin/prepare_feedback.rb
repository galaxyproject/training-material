...
data = data
  ...
  .select{|x| !x[:timestamp].nil? }
  # remove rows with NaN on note, pro and con
  .select{|x| !( x[:rating].nil? && x[:pro].nil? && x[:con].nil?) }
  # Ensure that they properly have a tutorial name and title. If this returns
  # NIL, then we'll ignore those.
  .select{|x| TITLE_TOPIC.match(x[:tutorial_topic])}
  # Various cleanups/extractions
  .each{|x|
  ...
  .select{|x| not DEAD_TUTORIALS.include? x[:tutorial] }
