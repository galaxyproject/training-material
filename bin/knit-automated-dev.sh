#!/bin/bash
DIR=/tmp/tools-from-scratch
op="$1"

declare -a tutorials
tutorials=(dev/tool-from-scratch)
...
if [[ "$op" == "export" ]]; then
	...
	cat > ${DIR}/0-commit-0000-root-commit.patch <<-EOF
		...
		+# Tools From Scratch
		+
		+This is built from [the GTN's library of dev training](https://training.galaxyproject.org/training-material/topics/dev/)
		+
		+Extra | Data
		+--- | ---
