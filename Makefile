...
check-html: build ## validate HTML
	bundle exec htmlproofer --assume-extension --http-status-ignore 405,503,999 --url-ignore "/.*localhost.*/","/.*vimeo\.com.*/","/.*gitter\.im.*/" --file-ignore "/.*\/files\/.*/","/.*\/node_modules\/.*/" ./_site
.PHONY: check-html

check-links-gh-pages:  ## validate HTML on gh-pages branch (for daily cron job)
	bundle exec htmlproofer --assume-extension --http-status-ignore 405,503,999 --url-ignore "/.*localhost.*/","/.*vimeo\.com.*/","/.*gitter\.im.*/" --file-ignore "/.*\/files\/.*/" .
