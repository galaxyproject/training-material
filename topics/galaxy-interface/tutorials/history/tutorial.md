...
![Searches for dataset](../../images/basic-search.png "Searching for datasets.")

### Clearing a search

You can clear a search and show all visible datasets by clicking the round 'X' button in the right of the search bar
or - while entering text in the search bar - hitting the escape key (<kbd>esc</kbd>).
...
> ### {% icon comment %} Keyword searches can be combined.
...
`tag=experiment1`                                  | for searching on (a partial) dataset tag. You can repeat to search for more tags.
...
![Advanced search](../../images/adv-search.png "Advanced search")
...
4. In the search bar, again, enter: `database=hg19 format=BAM name=output` and hit enter.
...
7. You can now perform some action on those two sets of datasets.
