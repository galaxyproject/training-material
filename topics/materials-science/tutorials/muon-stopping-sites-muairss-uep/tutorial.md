...
interpretation of experimental results, and such a problem has been commonly
tackled by using computational tools. 
...
- At that point, Coulomb forces start driving the muon, until it eventually
  ...
  electrostatic potential energy.  It may also be the case that the muon grabs
  an electron, becomes neutral muonium, and stops at a local or global minimum
  for the sample's full potential energy. Once it stopped, the muon may spend
...
Where these minima of the potential actually are isn't always obvious. For
example, in organic molecules the mechanism is often the same as for regular
hydrogenation. The muon, having captured an electron and formed a muonium
pseudo-atom, ends up breaking a high electronic density site, such as a double
or triple bond, attaching itself to the molecule and creating an organic radical
with its corresponding unpaired electron. However, there are certain crystalline
structures where predicting the potential minima can be a lot harder. In
lithium fluoride {% cite Liborio2018 %}, the potential's minima are flat and
wide and current computer simulations fail to predict the muon stopping site. 

In this workflow, we show a basic example of how to find some potential stopping
sites in a crystalline silicon sample using the {UEP} method. This method
requires only a single {DFT} calculation, which is used to relax the structure
and compute the electronic density of the host material
{% cite Sturniolo2020 %}. In this example, we used the CASTEP code for running
our {DFT} calculations. The electronic structure, in turn, is used to calculate
the minima of the host material’s electrostatic potential and to estimate
potential stopping sites for the diamagnetic muon, relying on the approximation
that the muon’s presence does not significantly affect its surroundings.
...
# Preparation steps
First, we need to load into Galaxy the necessary files for running a stopping
site calculation on crystalline silicon. First, we need the files from the
CASTEP {DFT} simulation. These are the CASTEP structural file `*.cell`, the
charge density file `*.den_fmt` and the CASTEP output file `*.castep`.  Then, we
need the file containing the parameters that all of the PyMuonSuite tools will
use for running: `*.yaml`.

We can run the workflow to estimate the muon stopping site either by hand,
running each PyMuonSuite tool individually, or automatically, running the whole
workflow in one go.
...
As we mentioned before, we can execute the entire workflow in one go, which will
generate stopping sites based on our three input files (generating the `*.yaml`
file in the process). Or, alternatively, the four tools that comprise the
workflow can be run one by one, or be included in other workflows. To aid
understanding, the steps below describe how to run each tool in detail: what
parameters they take and what exactly they do.

## UEP Stage 1: Create configuration file 

The first tool in the workflow, PyMuonSuite {AIRSS} Configure, handles the
configuration of all the subsequent tools in one go. When used outside a
workflow, Galaxy provides a UI with default values and help for the possible
parameters used by the various PyMuonSuite operations
{% cite Sturniolo_pymuon-suite_2022 %}. For this example many of these are
either left to their default values or are not specifically relevant to the
{UEP} use case. Those that are important have some further explanation.
...
>    > the input to step 4.  But this is not part of the UEP method.
>    {: .comment}
>
...
Given an input structural file, `*.cell`, and a parameter file `*.yaml`, this
tool generates  a set of duplicate structures, each with a single muon added in
a random location on the `*.cell` structural file. These are known as "muonated"
structures.
...
displace those muons in each structure to minimise the electrostatic force on
each one of them. The PyMuonSuite {AIRSS} {UEP} Optimise tool minimises those
analytical classical electrostatic forces with respect to the muon position,
using the {UEP} calculated for the original `*.cell` structure with the CASTEP
calculation.
...
The output of this tool is a zipped folder containing the optimised muonated
structures. We can then use this with the next tool in our workflow.

## UEP Stage 4: Clustering

Finally, we can cluster the optimised positions: each one of the cluster founds
becomes part of a list of potential stopping sites.
...
The final `Cluster report` will contain a human readable summary of the
clustering results, with the underlying raw data also provided.
...
> we also report the average energy corresponding to that cluster and which is
> the structure, within that cluster, with the minimum energy. This can help
> inform which site(s) are most favourable for the muon.
>
{: .details}

While we may have generated tens of structures with muons in random positions,
we should now see only a few potential stopping sites. This is because the
optimisation proceeds to cluster the muons while minimising the forces acting on
them. These new positions are likely to overlap, corresponding to local energy
minima.
...
We have used tools from PyMuonSuite to determine the possible stopping sites of
implanted muons in a simple structure. It is also possible modify the first
stage of this workflow to use different settings, or to create the settings
file used by PyMuonSuite with
...
separately and modify the workflow to take it the `*.yaml` file as an additional
input replacing the first stage. The Galaxy implementation has explanations of
the various settings, and sensible values set by default.
