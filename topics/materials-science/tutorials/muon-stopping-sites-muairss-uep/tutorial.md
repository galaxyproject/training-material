...
This tutorial goes into the detals of how to estimate the muon stopping 
...
![Workflow for Finding the µ+ stopping site](../../images/Workflow_for_finding_mu+.png)
...
![Galaxy used for this Process](../../images/2GalaxyTools.png)
...
![Galaxy Workflow](../../images/GalaxyWorkflow.png)
...
## Density Functional Theory (DFT) calculation of the Pure Host Material
...
the forces are calculated analitically using data obtained from the DFT simulation of the unperturbed host material: 
...
![Calculate and Relax Electrostatic Forces on the Muons](../../images/relax_electrostatic_forces_on_muons.png) 
...
![Clustering Analysis to Classify UEP's Minima as Potential Stopping Sites](../../images/cluster_relaxed_structures_mu+_stopping_sites.png) 

The first setp for running the clusterin procedure is to select the `PyMuonSuite AIRSS Cluster` Galaxy tool on
...
available: `hierarchical` and `k-means` clustering. We recomend using the hierarchical clustering first, chosing different 
...
a CASTEP DFT simulation, which can be used to analise things such as the distortions caused by the muon in the host 
material.  

![Further Analysis of the Potential Stopping Sites](../../images/DFT_supercell_muonated_structures.png) 
