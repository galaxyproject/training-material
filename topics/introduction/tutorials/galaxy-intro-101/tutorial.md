...
![Galaxy interface screenshot showing History panel on the right, tools panel on the left, and main panel in the center](../../images/galaxy_interface.png "<b>Galactic triptych</b>: the three panels of Galaxy interface: <em>Tools</em>, <em>Center Pane</em>, and <em>History</em>. Here you also see the 'Activity bar' that can be turned on and off (Don't see the Activity Bar? See a Tip below).")
{: .comment}
...
> >
> > 1. Search for **UCSC Main** table Browser on the tools search bar {% icon tool %}:
> >
...
> > Again, search for **UCSC Main** table Browser on the tools search bar {% icon tool %} and set the following parameters
> >
...
>
> To find intersection we will be using `{% tool [bedtools intersect intervals](toolshed.g2.bx.psu.edu/repos/iuc/bedtools/bedtools_intersectbed/2.31.1+galaxy0) %} from [BEDTools](https://bedtools.readthedocs.io/en/latest/content/overview.html) package.
>
> 1. {% tool [bedtools intersect intervals](toolshed.g2.bx.psu.edu/repos/iuc/bedtools/bedtools_intersectbed/2.31.1+galaxy0) %} find overlapping intervals in various ways:
>
...
>
> 1. {% tool [Sort](toolshed.g2.bx.psu.edu/repos/bgruening/text_processing/tp_sort_header_tool/9.5+galaxy0) %} data in ascending or descending order:
>
...
> 3. Click on the Galaxy name at the top left to return to the main analysis window
>
