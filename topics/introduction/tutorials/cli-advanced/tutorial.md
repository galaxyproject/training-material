...
![Redirects and Pipes of different commands: 'wc -l *.pdb' will direct the output to the shell. 'wc -l *.pdb > lengths' will direct output to the file 'lengths'. 'wc -l *.pdb | sort -n | head -n 1' will build a pipeline where the output of the 'wc' command is the input to the 'sort' command, the output of the 'sort' command is the input to the 'head' command and the output of the 'head' command is directed to the shell](../../images/carpentries-cli/redirects-and-pipes.svg)
