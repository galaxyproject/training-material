...
In this tutorial we will use an assembly based on PacBio and Arima2 Hi-C data generated by the Aquatic Symbiosis Genomics Project (https://www.ebi.ac.uk/ena/browser/view/GCA_947172415). We will also use samples supplied by the ASG Sponges-as-Symbiont Hub (https://www.aquaticsymbiosisgenomics.org/collaborators/the-genomic-basis-of-sponge-microbe-symbioses/).
...
HISAT2 is currently one of the fastest RNA-seq mapper available. The tool will use hierarchically indexing methods to align reads to a reference genome. It then provides the alignment output in SAM file format which we will then use as an input for Blobtoolkit (http://daehwankimlab.github.io/hisat2/).
...
> <hands-on-title> Counting k-mers </hands-on-title>
>
...
> <hands-on-title> Generate histogram </hands-on-title>
>
> 1. {% tool [Meryl](toolshed.g2.bx.psu.edu/repos/iuc/meryl/meryl/1.3+galaxy6) %} with the following parameters:
