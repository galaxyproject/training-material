...
zenodo_link: https://zenodo.org/records/13332983
questions:
...
In this tutorial, we will learn how to separate these contaminants from a genome assembly of a vertebrate. It will also include the separation of mitochondrial DNA (Mitochondrial Genome Assembly can be found in another tutorial) so the genome assembly obtained in the end contain only nuclear DNA.
...
For this tutorial, we are using the genome assembly of the Zebra Finch (*Taeniopygia guttata*) generated by the [Vertebrate Genome Project](https://vertebrategenomesproject.org/). We downsized the assembly to 50 of the largest scaffold to keep the running time reasonable, and added some contaminants.
...
>
>    ```
>    https://zenodo.org/records/13332983/files/Contaminated_assembly.fasta
>    ```
>
...
Genome masking is the technique of hiding the regions of the genome that cary low complexity, such as repeats. Masking a genome reduces the noise and improves the efficacy of the decontamination by emphasizing the regions that contribute the most to the classification ({% cite saini2016gene %}, {% cite maskedref-website %}).
...
- **Hard Masking** The masked regions are replaced by a wildcard letter:  `N`s for nucleotide sequences, `X`s for proteins. Use this technique if you are unsure whether the tools you will be using understands to ignore lower case letters.  
...
The first step is to ensure the sequences in our assembly are in upper cases. This will ensure the soft masking can be done properly. We will then convert the lower case letter to `N`s to hard mask the sequences for Kraken2 and Blast.

> <details-title> Why do we need to convert everything to upper case? </details-title>
>
> 1. The sequences : Some fasta sequences are sometimes in lower case. In that situation the soft masking step won't work because there will be not differences between the masked sequences and the rest.
> 2. The sequence names : A bit later in the analysis, we will convert the soft masked sequences to hard masked sequences. This is done by converting lower case letter to N's. If the sequences names are not in upper case, `N`s would be introduced there as well. 
>
{: .details}
...
>
> 2. Rename your file `Upper case assembly`
>
...
>        - {% icon param-file %} *"Nucleotide FASTA subject file to use instead of a database"*: `Upper case assembly` (output of **Text transformation** {% icon tool %})
>    - *"DUST level"*: `40`
>    - *"Output format"*: `FASTA`
>
...
>    - *"Select a Kraken2 database"*: `PlusPF`
>
>
{: .hands_on}
...
To identify our contaminants, we only need the informations from the first three columns, and only for the sequences that are classified. So we will start by extracting our three columns and then filter out unclassified sequences. 
...
In order to remove the contaminant sequences from our assembly, we need a list of the contaminants name, that we will get by extracting the contaminant IDs from the Kraken outputs. Remember at this point that we transformed the sequence headers to upper case to prevent issues during the genome masking. We will need to convert them back to lower case so that they match the original assembly.  
...
Now that we have a list of mitochondrial scaffolds, we will transform the scaffold names to lowercase to match the original assembly, as we did with the contaminant sequences.


> <hands-on-title> Convert mitochondrial sequence names to lower case to match the original assembly </hands-on-title>
>
> 1. {% tool [Text transformation](toolshed.g2.bx.psu.edu/repos/bgruening/text_processing/tp_sed_tool/1.1.1) %} with the following parameters:
>    - {% icon param-file %} *"File to process"*: `Parse mitochondrial blast on data X` (output of **Parse mitochondrial blast** {% icon tool %})
>    - *"SED Program"*: `s/[A-Z]/\L&/g`
>    - *"Advanced Options"*: `Hide Advanced Options`
>
...
>        - {% icon param-file %} *"Exclude specific intervals"*: `Sequences to remove` (output of **Concatenate datasets** {% icon tool %})
