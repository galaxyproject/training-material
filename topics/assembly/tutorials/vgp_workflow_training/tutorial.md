...
> <details-title>Data upload</details-title>
>
...
> 2. Click in the {% icon workflow-run %} **Run workflow** button corresponding to `VGP genome profile analysis`
> 3. In the **Workflow: VGP genome profile analysis** menu:
>   - {% icon param-collection %} "*Collection of Pacbio Data*": `7: HiFi_collection`
>   - "*K-mer length*": `31`
>   - "*Ploidy*": `2`
>   - "*Species name (latin name)*": `Saccharomyces cerevisiae`
...
> <warning-title>Did MitoHiFi fail?</warning-title>
>
> MitoHiFi can sometimes fail if your input data does not contain enough mitochondrial reads, or if there is not a reference mitochondrial assembly for the given species in NCBI.
>
{: .warning}
...
**Notice that the workflow produces two "usable GFA" files, one for hap1 and one for hap2. These are the hifiasm output GFAs after being run through gfastats to make them compliant with GFA 1.2 spec, allowing for the later GFA reconciliation steps in the pipeline.**
...
>
> 1. Click in the **Workflow** menu, located in the top bar
> 2. Click in the {% icon workflow-run %} **Run workflow** button corresponding to `VGP purge assembly with purge_dups pipeline`
...
>
> 1. Click in the **Workflow** menu, located in the top bar
> 2. Click in the {% icon workflow-run %} **Run workflow** button corresponding to `VGP hybrid scaffolding with Bionano optical maps`
> 3. In the **Workflow: VGP hybrid scaffolding with Bionano optical maps** menu:
>   - {% icon param-file %} "*Bionano data*": `1: Bionano_dataset`
>   - {% icon param-file %} "*Input GFA*": `[hap1/hap2] usable GFA` (note: the data numbers may differ in your workflow)
...
>
> 1. Click in the **Workflow** menu, located in the top bar
> 2. Click in the {% icon workflow-run %} **Run workflow** button corresponding to `VGP hybrid scaffolding with HiC data`
> 3. In the **Workflow: VGP hybrid scaffolding with HiC data** menu:
>   - {% icon param-file %} "*Input GFA*": either `usable GFA` or `Bionano GFA`
...
>   - *Restriction enzymes*: select `Arima 2.0`
> 4. Click in the <kbd>Run workflow</kbd> button
{: .hands_on}
