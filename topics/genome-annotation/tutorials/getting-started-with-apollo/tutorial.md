...
On the left side of the embedded JBrowse instance is a checklist-like column titled “Available Tracks;” this contains the evidence tracks currently available to the account in use. Evidence tracks will appear upon the execution of structural and functional workflows.
The first evidence tracks to be used will be those from the structural workflow; these gene call tracks are *GeneMarkS*, *MetaGeneAnnotator*, and *Glimmer3*. Selecting these will yield predicted genes.
...
> Having the gene tracks on in combination with the Apollo panel and the Galaxy columns can be overwhelming and distracting from the annotation. Selecting the arrow in the top left corner of the Apollo panel will close it. There are the same kind of arrows in the bottom right corner of the History column and in the bottom left corner of the Tools column; clicking on these will collapse the columns and further expand the annotation screen. 
> The Available Tracks column can be shrunken by clicking and dragging the right edge of the columnAdditionally, zooming down to the single gene level can greatly assist in focusing on annotation. 
{: .tip}
