...
> This image above originates from this report: [Xu J, Hendrix RW, Duda RL. 2004. Conserved translational frameshift in dsDNA bacteriophage tail assembly genes. Mol Cell 16:11–21.](https://www.sciencedirect.com/science/article/pii/S1097276504005398?via%3Dihub) This article contains greater detail about these programmed translational frameshifts, and offers examples of frameshifts differing from the -1 programmed translational frameshifts described in this tutorial.
{: .comment}
...
> ### {% icon tip %} Note That…
> This portion of the annotation process can **_NOT_** be completed unless both the [structural annotation workflow] and [functional annotation workflow] have been successfully run.
> <!-- LINK TUTORIALS UPON COMPLETION -->
{: .tip}

![](../../images/annotating-tmp-chaperone-frameshifts-screenshots/4_view_genome.png)

It is useful to close the history and tools columns on the right and left side of the screen (respectively). Keep the Available Tracks and Apollo Annotations windows open on the left and right sides of the screen; this will offer greater ease in the search. Clicking on “Length” at the top of the table will organize the genes by their length in base pairs. Classically, the TMP will be the longest gene in the genome; however, tail fiber proteins also tend to rank with the longest genes in a phage’s genome. Browse all of the genes by clicking the arrows above the table. Evidence tracks can be used to help determine TMP from tail fiber proteins. Clicking on that gene product will reveal more details about the gene below, including the specific location on the genome. Locate and zoom into the gene in the center window.

![](../../images/annotating-tmp-chaperone-frameshifts-screenshots/5_zoomed_in_gene.png)

Before annotating the gene as the TMP for the phage, it is important to use the evidence tracks to search for supporting evidence. Locate the Protein options underneath the BLAST section of the Available Tracks column. Turn on the Canonical Phages track. If that does not yield any results, turn that off and turn on SwissProt, followed by TrEMBLE and NR if there still aren’t any viable hits. Here, SwissProt was able to provide a useful result.

![](../../images/annotating-tmp-chaperone-frameshifts-screenshots/6_swissprot_hit.png)

Upon determination of the TMP location, the annotator will look directly upstream for the TMP chaperone genes. It is likely that these genes were previously called, but the frameshift was *not*. Turn on the Possible Frame Shifts track underneath the Phage subsection of the Sequence Analysis section. Using the resulting evidence hits as a guide, look at the genes closely upstream of the TMP location; zoom in so that the nucleotides, triplet codons, and reading frames are given above the User-Created Annotations track.

![](../../images/annotating-tmp-chaperone-frameshifts-screenshots/7_possible_frameshift_location.png)

Although the second gene (above, CDS.0.116_0.388872507629) have a good start codon, looking upstream reveals that there is a very poor (debatably nonexistent) Shine-Dalgarno sequence. This will most likely be the case for chaperones and frameshifts - the second gene will have a very poor Shine-Dalgarno sequence, or it won’t have one at all.
...
> ### {% icon tip %} Note that…
> The slippery sequence will not always include one or two lysine residues. Phenylalanine (F; one or two), glycine (G; one or two), glutamate (E; most likely only one), leucine (L; most likely only one), and asparagine (N; most likely only one) are all possible amino acids in -1 frameshifts. Refer back to the image containing the canonical phage slippery sequences above, cited from [this report.](https://www.sciencedirect.com/science/article/pii/S1097276504005398?via%3Dihub)
{: .tip}
...
> ### {% icon comment %} Lacking Shine-Dalgarno Sequences
...
> In the case of a missing gene between the TMP and chaperone, genes from *Glimmer3* or *MetaGeneAnnotator* can be called/manipulated. Here, the PBSX prophage genome is is adjusted.
>
...
>
> 1. Click on the first TMP chaperone frameshift gene in the User-created Annotations; this will border the gene in red. Hover over the right end of the gene until the cursor becomes and arrow pointing to the right.
>
...
The TMP chaperone and TMP chaperone frameshift  products have been annotated, and they must be named and have the details adjusted accordingly.
