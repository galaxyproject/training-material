...
To illustrate the process to identify AMR gene in a bacterial genome, we take an assembly of a bacterial genome generated by following a [bacterial genome assembly tutorial]({% link topics/assembly/tutorials/mrsa-illumina/tutorial.md %}) from data produced in "Complete Genome Sequences of Eight Methicillin-Resistant *Staphylococcus aureus* Strains Isolated from Patients in Japan" {% cite Hikichi_2019 %}. 
...
1. `summary.tsv`:  A summary of all detected AMR genes/mutations/plasmids/sequence type in each genome, one genome per line and the following columns:
   ...
   * __Quality Module Feedback__: The isolate/genome file(s) detailed feedback for the quality metrics
    ...
    > > 3. amikacin, gentamicin, tobramycin, spectinomycin, erythromycin, azithromycin, penicillin, tetracycline 
...
If you already ran **Bakta** on your contigs (maybe after another [tutorial]()), you can just upload here the GFF generated by **Bakta**.
...
>
>    ```
>    {{ page.zenodo_link }}/files/DRR187559_1.fastq.gz
>    {{ page.zenodo_link }}/files/DRR187559_2.fastq.bz2
>    ```
>
> 2. {% tool [Bowtie2](toolshed.g2.bx.psu.edu/repos/devteam/bowtie2/bowtie2/2.5.0+galaxy0) %} with the following parameters:
>    - *"Is this single or paired library"*: `Paired-end`
>        - {% icon param-file %} *"FASTA/Q file #1"*: Forward reads
>        - {% icon param-file %} *"FASTA/Q file #2"*: Reverse reads
