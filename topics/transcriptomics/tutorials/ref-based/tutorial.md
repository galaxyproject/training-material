...
> > 2. To display the most abundantly detected feature, we need to sort the table of counts. This can be done using the **Sort** {% icon tool %} tool, by sorting the table on the second column (which contains the number of reads = counts) in descending order. The result of sorting the table on column 2 reveals that FBgn0000556 is the feature with the most counts (around 128,741 in `GSM461177` and 127,416 in `GSM461180`).
> > 
...
3. A table with the differentially expressed genes (from the list we provided) associated to the GO terms
...
**goseq** can also be used to identify interesting KEGG pathways:
...
1. A big table with the KEGG terms and some statistics
    ...
    > 2. How many KEGG pathways terms are over-represented at adjusted P value < 0.05?
    > 3. What is the first 3 KEGG pathways?
    >
    > > ### {% icon solution %} Solution
    > >
    > > 1. The file has 127 lines including an header, so 126 KEGG pathways have been identified.
    > > 2. 17 KEGG pathways (13.39%) are over-represented, using **Filter** on c8 (over-represented)
    > > 3. The first 3 KEGG pathways are:
    > >    - `01100`: Metabolic pathways, all metabolic pathways
    > >    - `00010`: Glycolysis / Gluconeogenesis 
    > >    - `00480`: Glutathione metabolism
    > {: .solution}
    {: .question}

2. A table with the differentially expressed genes (from the list we provided) associated to the KEGG pathways
...
>    - *"Cut columns"*: `c1`
>    - *"Delimited by"*: `Tab`
>    - {% icon param-file %} *"From"*: output of **Filter** on last **goseq** to get the over-represented KEGG pathways 
>
> 4. **Select first lines** {% icon tool %} with
>    - {% icon param-file %} *"File to select"*: output of the last **Cut** 
>    - *"Operation"*: `Keep first lines`
>    - *"Number of lines"*: `3`
>
> 4. **Select first lines** {% icon tool %} with
>    - {% icon param-file %} *"File to select"*: output of the last **Select first lines** 
>    - *"Operation"*: `Keep everything from this line on`
>    - *"Number of lines"*: `2`
>
> 5. Rename to `Top 2 KEGG pathways`
>
> 4. **Pathview** {% icon tool %} with
>    - *"Number of pathways to plot"*: `Multiple`
>      - {% icon param-file %} *"KEGG pathways"*: `Top 5 KEGG pathways`
