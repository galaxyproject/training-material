...
>
> 2. **Concatenate datasets** tail-to-head (cat) {% icon tool %} to add this header line to the **Filter** output:
...
>
>    > ### {% icon comment %} Comment
>    > Note that there are two **Cut** tools, but the **Cut** *columns from a table (cut)* tool should *not* be used.
>    >
>    {: .comment }
>
{: .hands_on}
