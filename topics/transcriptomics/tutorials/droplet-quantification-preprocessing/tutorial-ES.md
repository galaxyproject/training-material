...
Hemos reducido la resolución de los archivos FASTQ para poder hacer el análisis un poco más rápido, esto debido a la enorme cantidad de datos contenidos en estos archivos. Una vez que hemos aclarado este punto, todavía tienes que mapear enormes cantidades de lecturas al inmenso genoma murino, así que ¡ve por una taza de café y prepárate para iniciar el análisis!
...
En esta sección te mostraremos los principios de la fase inicial del análisis de Secuenciación de ARN de células únicas, que se basa en  generar mediciones de expresión en una matriz. Por términos prácticos nos referiremos a este análisis por sus siglas en inglés scRNA-Seq (single-cell RNA-Seq). Nos concentramos en la metodología basada en goteo (en lugar de las metodologías basadas en placas), ya que es el proceso con más diferencias con respecto a los enfoques convencionales desarrollados para secuenciación de ARN general.
...
Utilizamos la tecnología Drop-seq para este estudio. Sin embargo, el análisis que abordaremos en este tutorial es casi idéntico al que se realiza al utilizar la tecnología 10x. Estos datos no han sido curados cautelosamente, los datos del tutorial son algo caóticos ya que proviene de un experimento real. Necesitan filtrado urgentemente ya que contiene ARNs que le agregan ruido de fondo, sin embargo, te darán la oportunidad de practicar un análisis como si fuera el tuyo propio.
...
> > 2. Inspecciona {% icon galaxy-eye %} el archivo {% icon param-file %} 'quants_mat_rows.txt', y podrás ver que contiene `2163` líneas. Los renglones hacen referencia a las células en la matriz de correspondencia de células a genes. Según esta estimación (aproximada), tu muestra contiene 2163 células.
> >
> {: .solution}
>
{: .question}
...
Alevin da salida a un formato MTX, el cual puede ser la entrada del paquete dropletUtils y correrse en emptyDrops. Desafortunadamente la matriz tiene una orientación equivocada. La matriz no puede ser procesada por herramientas cuya entrada son archivos como los que produce el software 10X. Necesitamos ‘transformar’ la matriz de tal forma que las células esten en columnas y los genes en renglones.
...
> ### {% icon question %} Pregunta
>
> ¿Cuántos códigos de barras de células quedan después de correr emptyDrops? ¿Por qué será esto?
>
>   > ### {% icon tip %} Pista
...
{% icon congratulations %} ¡Felicidades! Tu objeto está listo para el pipeline de scanpy. Ahora puedes verificar tu trabajo comparándolo con el mostrado en este [example history](https://humancellatlas.usegalaxy.eu/u/wendi.bacon.training/h/pre-processing-with-alevin---part-1---answer-key).
