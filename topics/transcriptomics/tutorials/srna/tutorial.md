...
>
> 1. **FastQC** {% icon tool %}: Run **FastQC** on each collection using the following parameters:
>    - "Short read data from your current history" to `Dataset collection` tab and then select the blank RNAi sRNA-seq dataset collection
>
...
>
> 1. **FASTQ Groomer** {% icon tool %}: Run **FASTQ Groomer** on each collection using the following parameters:
>    - "File to groom" to `Dataset collection` tab and select the control RNAi sRNA-seq dataset collection
>    - "Input FASTQ quality scores type"to `Illumina 1.3-1.7`
>
{: .hands_on}
...
>
> 1. **Trim Galore!** {% icon tool %}: Run **Trim Galore!** on each collection of FASTQ files with the following parameters:
>    - "Is this library paired- or single-end?" to `Single-end`
>    - "Reads in FASTQ format" to `Dataset collection` tab and select the blank RNAi sRNA-seq dataset
>    - "Trimming reads?" to `Illumina universal`
>    - "Trim Galore! advanced settings" to `Full parameter list`
>    - "Trim low-quality ends from reads in addition to adapter removal" to `0`
>    - "Overlap with adapter sequence required to trim a sequence" to `6`
>    - "Discard reads that became shorter than length INT"to `12`
...
>
> 1. **HISAT2** {% icon tool %}: Run **HISAT2** on each collection of trimmed reads with the following parameters:
>    - "Single end or paired reads?" to `Individual unpaired reads`
>    - "Reads" to `Dataset collection` tab and select the blank RNAi sRNA-seq dataset of trimmed FASTQ files
>    - "Source for the reference genome to align against" to `Use a genome from history`
>    - "Select the reference genome" to `Dmel_rRNA_sequences.fa`
>    - "Spliced alignment parameters" to `Specify spliced alignment parameters`
>    - "Specify strand-specific information" to `Second Strand (F/FR)`
>
> 1. Repeat for the *Symplekin* RNAi dataset collection
>
>    > ### {% icon comment %} Comment
...
>    - "SAM or BAM file to filter" to `Dataset collection` tab and select the blank RNAi sRNA-seq dataset of aligned HISAT2 BAM files
>    - "Filter on bitwise flag" to `Yes`
>    - "Only output alignments with all of these flag bits set" to `The read in unmapped`
>
> 1. Repeat for the *Symplekin* RNAi dataset collection
>
...
>    - "Convert the following BAM file to FASTQ" to `Dataset collection` tab and select the blank RNAi sRNA-seq dataset of filtered HISAT2 BAM files
>
> 1. Click "Execute"
> 1. Repeat for the *Symplekin* RNAi dataset collection
>
...
>    - "Single end or paired reads?" to `Individual unpaired reads`
>    - "Reads" to `Dataset collection` tab and select the blank sRNA-seq dataset of non-rRNA FASTQ files
>    - "Source for the reference genome to align against" to `Use a genome from history`
>    - "Select the reference genome" to `Dmel_miRNA_sequences.fa`
>    - "Spliced alignment parameters" to `Specify spliced alignment parameters`
>    - "Specify strand-specific information" to `Second Strand (F/FR)`
> 1. Repeat for the *Symplekin* RNAi dataset collection
>
...
>    - "FASTQ File" to `Dataset collection` tab and select the blank RNAi sRNA-seq dataset of non-r/miRNA FASTQ files
>    - "Match Reads" to `Insert Match Reads`
>    - "Match Reads by" to `Sequence Content`
>    - "Match by" to`^.{12,19}$|^.{23,50}$`
>    - "Manipulate Reads" to `Insert Manipulate Reads`
>    - "Manipulate Reads on" to `Miscellaneous actions`
>    - "Miscellaneous Manipulation Type" to `Remove Read`
> 1. Repeat **Manipulate FASTQ** {% icon tool %} for the *Symplekin* RNAi dataset collection
...
>    - "Match Reads" to `Insert Match Reads`
>    - "Match Reads by" to `Sequence Content`
>    - "Match by" to `^.{12,22}$|^.{30,50}$`
>    - "Manipulate Reads" to `Insert Manipulate Reads`
>    - "Manipulate Reads" to `Miscellaneous actions`
>    - "Miscellaneous Manipulation Type" to `Remove Read`
> 1. Repeat **Manipulate FASTQ** {% icon tool %} for the *Symplekin* RNAi dataset collection
...
>
> 1. **Salmon** {% icon tool %}: Run **Salmon** on each collection of siRNA reads (20-22nt) to quantify the abundance of *antisense* siRNAs at relevant targets using the following parameters:
>    - "Select a reference transcriptome from your history or use a built-in index?" to `Use one from the history`
>    - "Select the reference transcriptome" to the reference mRNA and TE fasta file
>    - "The size should be odd number" to `19`
>    - "FASTQ/FASTA file" to `Dataset collection` tab and select the Blank RNAi siRNA (20-22nt) reads
>    - "Specify the strandedness of the reads" to `read 1 (or single-end read) comes from the reverse strand (SR)`
>    - "Additional Options" to `yes`
>    - "Incompatible Prior" to `0`
>
> 1. Repeat **Salmon** {% icon tool %} for the *Symplekin* RNAi siRNAs (20-22nt) reads dataset collection
...
>    - "Specify the strandedness of the reads" to `read 1 (or single-end read) comes from the forward strand (SF)`
> 1. Repeat **Salmon** {% icon tool %} for the *Symplekin* RNAi siRNAs (20-22nt) reads dataset collection
>
{: .hands_on}
...
>    - "Specify a factor name" to `RNAi`
>    - "1: Factor level: Specify a factor level" to `Symplekin`
>    - "1: Factor level: Counts file(s)" to `Dataset collection` tab and select the Symplekin RNAi siRNA counts from reverse strand (SR)
>    - "2: Factor level: Specify a factor level" to `RNAi`
>    - "2: Factor level: Counts file(s)" to `Dataset collection` tab and select the Symplekin RNAi siRNA counts from reverse strand (SR)
>    - "Choice of Input data" to `TPM values (e.g. from sailfish and salmon)`
>    - "Tabular file with Transcript - Gene mapping" to reference tx2g (transcript to gene) file (available through zenodo link)
>    - "Output normalized counts table" to `Yes`
>
> 1. Repeat **DESeq2** {% icon tool %} for the *sense* siRNAs at mRNA and TE features changing the following parameters:
>    - "1: Factor level: Counts file(s)" to `Dataset collection` tab and select the Symplekin RNAi siRNA counts from forward strand (SF)
>    - "2: Factor level: Counts file(s)" to `Dataset collection` tab and select the Symplekin RNAi siRNA counts from forward strand (SF)
>
...
>    - "Filter" to the `DESeq2` result file from testing *antisense* (SR) siRNA abundances
>    - "With following condition" to `c7<0.05`
>
> 1. Repeat **Filter** {% icon tool %} for the *sense* siRNAs at mRNA and TE features changing the following parameters:
>    - "Filter" to the `DESeq2` result file from testing *sense* (SF) siRNA abundances
>
