...
And that's it! 
However, if we do small modification to metadata, we can use the output dataset in the workflow we created in the [previous tutorial]({% link topics/single-cell/tutorials/scrna-case_basic-pipeline/tutorial.md %}) for the AnnData input file! Let's see how it would work:
