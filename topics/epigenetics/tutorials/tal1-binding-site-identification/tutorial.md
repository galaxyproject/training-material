...
zenodo_link: 'https://zenodo.org/record/197100'
answer_histories:
...
objectives:
  - Inspect read quality with falco
...
>
> 3. Add tags to describe the datasets. Multiple tags can be added to each dataset. 
>   - For eg., for `G1E_input_R1_downsampled_SRR507859.fastqsanger` file, you could add tags called `#G1E`, `#Input` and `#Rep1` to the R1 file and a tag called `#SRR507860_R2` to the R2 file. 
>   - Another eg., for `Megakaryocytes_Tal1_R2_downsampled_SRR549007.fastqsanger` file, you could add tags called `#Megakaryocytes`, `#Tal1` and `#Rep2`.
>
>    {% snippet faqs/galaxy/datasets_add_tag.md %}
>
> 4. Examine the data in a FASTQ file by clicking on the {% icon galaxy-eye %} (eye) icon.
>
...
> 1. {% tool [Falco](toolshed.g2.bx.psu.edu/repos/iuc/falco/falco/1.3.2+galaxy0) %}: Run Falco on each FASTQ file to assess the quality of the raw data. An explanation of the results can be found on the [FastQC web page](https://www.bioinformatics.babraham.ac.uk/projects/fastqc/).
>    - {% icon param-files %} *"Short read data from your current history"*: The uploaded fastqsanger files.
>
>    ![fastqbefore](../../images/tal1/falco_before.png "Sequence quality per base generated by Falco before end trimming.")
>
>    > <question-title></question-title>
>    >
...
>
> 1. {% tool [Trimmomatic](toolshed.g2.bx.psu.edu/repos/pjbriggs/trimmomatic/trimmomatic/0.39.2+galaxy2) %}: Run Trimmomatic to trim low-quality reads.
...
>    {: .tip}
>
> 2. {% tool [Falco](toolshed.g2.bx.psu.edu/repos/iuc/falco/falco/1.3.2+galaxy0) %}: Rerun Falco on each trimmed/clipped FASTQ file to determine whether low-quality and adaptor sequences were correctly removed.
...
>    {: .question}
>
>    ![fastqafter](../../images/tal1/falco_after.png "Sequence quality per base generated by Falco after end trimming.")
{: .hands_on}
...
>
> 1. {% tool [BWA](toolshed.g2.bx.psu.edu/repos/devteam/bwa/bwa/0.7.19+galaxy1) %}: Run BWA to map the trimmed/clipped reads to the mouse genome.
...
>    {: .question}
>
> 4. {% tool [Samtools idxstats](toolshed.g2.bx.psu.edu/repos/devteam/samtools_idxstats/samtools_idxstats/2.0.8) %}: Run idxstats to get statistics of the BWA alignments.
...
>
> 1. {% tool [multiBamSummary](toolshed.g2.bx.psu.edu/repos/bgruening/deeptools_multi_bam_summary/deeptools_multi_bam_summary/4.0.0+galaxy1) %}: Run multiBamSummary to get read coverage of the alignments.
>     - *"Sample order matters"*: `No`
>     - {% icon param-files %} *"Bam files"*: Select all of the aligned BAM files
>     - *"Bin size in bp"*: 1000
>
> 2. {% tool [plotCorrelation](toolshed.g2.bx.psu.edu/repos/bgruening/deeptools_plot_correlation/deeptools_plot_correlation/4.0.0+galaxy1) %}: Run plotCorrelation to visualize the results.
...
>
> 1. {% tool [plotFingerprint](toolshed.g2.bx.psu.edu/repos/bgruening/deeptools_plot_fingerprint/deeptools_plot_fingerprint/4.0.0+galaxy1) %}: Run plotFingerprint to assess ChIP signal strength.
...
>
> 1. {% tool [MACS2 callpeak](toolshed.g2.bx.psu.edu/repos/iuc/macs2/macs2_callpeak/2.2.9.1+galaxy0) %}: Run MACS2 callpeak with the aligned read files from the previous step as Treatment (TAL1) and Control (input).
...
MACS2 generates bedGraph and BED files that we will use to visualize read abundance and peaks, respectively, at regions MACS2 determines to be TAL1 peaks using pyGenomeTracks.

## Inspection of peaks and aligned data with pyGenomeTracks
...
>    {: .question}
>
>    ![runx1](../../images/tal1/pygenometracks_runx1.png "The Runx1 locus with TAL1 peaks identified.")
{: .hands_on}

## Inspection of peaks and aligned data with IGV
We show here an alternative to pyGenomeTracks, [IGV](http://software.broadinstitute.org/software/igv/).
...
>
> 1. {% tool [bedtools Intersect intervals](toolshed.g2.bx.psu.edu/repos/iuc/bedtools/bedtools_intersectbed/2.31.1+galaxy0) %}: Run bedtools Intersect intervals to find peaks that exist both in G1E and megakaryocytes.
...
>
> 2. {% tool [bedtools Intersect intervals](toolshed.g2.bx.psu.edu/repos/iuc/bedtools/bedtools_intersectbed/2.31.1+galaxy0) %}: Run bedtools Intersect intervals to find peaks that exist only in G1E.
>    - {% icon param-file %} *"File A to intersect with B"*: Select the TAL1 G1E narrow peaks BED file
>    - {% icon param-file %} *"File B to intersect with A"*: Select the TAL1 Megakaryocytes narrow peaks BED file
>    - *"Report only those alignments that \*\*do not\*\* overlap the BED file"*: `Yes`
>
> 3. {% tool [bedtools Intersect intervals](toolshed.g2.bx.psu.edu/repos/iuc/bedtools/bedtools_intersectbed/2.31.1+galaxy0) %}: Run bedtools Intersect intervals to find peaks that exist only in megakaryocytes.
...
>
> 1. {% tool [bamCompare](toolshed.g2.bx.psu.edu/repos/bgruening/deeptools_bam_compare/deeptools_bam_compare/4.0.0+galaxy1) %}: Run bamCompare to get the log2 read ratios between treatment and control samples.
...
>
> 1. {% tool [computeMatrix](toolshed.g2.bx.psu.edu/repos/bgruening/deeptools_compute_matrix/deeptools_compute_matrix/4.0.0+galaxy1) %}: Run computeMatrix to prepare data for plotting a heatmap of TAL1 peaks.
...
>
> 1. {% tool [plotHeatmap](toolshed.g2.bx.psu.edu/repos/bgruening/deeptools_plot_heatmap/deeptools_plot_heatmap/4.0.0+galaxy1) %}: Run plotHeatmap to create a heatmap for score distributions across TAL1 peak genomic regions in each cell type.
...
>
> 1. {% tool [computeGCbias](toolshed.g2.bx.psu.edu/repos/bgruening/deeptools_compute_gc_bias/deeptools_compute_gc_bias/4.0.0+galaxy0) %}: Run computeGCbias to determine the GC bias of the sequenced reads.
...
>    {: .question}
>
> 2. {% tool [correctGCbias](toolshed.g2.bx.psu.edu/repos/bgruening/deeptools_correct_gc_bias/deeptools_correct_gc_bias/4.0.0) %}: Run correctGCbias to generate GC-corrected BAM/CRAM files.
>
>    > <question-title></question-title>
>    >
