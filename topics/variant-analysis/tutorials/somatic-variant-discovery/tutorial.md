...
Human Copy Number Variations (hCNVs) are the result of structural genomic rearrangements that result in the duplication or deletion of DNA segments [(Alkan., et all 2011)](https://academic.oup.com/bib/article/17/2/185/1744035?login=true). These changes contribute significantly to human genetic variability, diseases, and somatic genome variations in cancer and other diseases [(Nam., et al 2016)](https://academic.oup.com/bib/article/17/2/185/1744035?login=true). hCNVs can be routinely investigated by genomic hybridisation and sequencing technologies
. There is a range of software tools that can be used to identify and quantify hCNVs. Unfortunately, locating hCNVs is still a challenge in standardising formats for data representation and exchange. Furthermore, the sensitivity, specificity, reproducibility, and reusability of hCNV detection and analysis research software varies. As a result, there is a need for the adoption of community-developed standards for data discovery and exchange. To address that ELIXIR developed [Beacon](https://beacon-project.io/) protocol and [GA4GH](https://www.ga4gh.org/) standards, as well as mechanisms for annotating, benchmarking, creating reproducible and sharable tools and workflows, such as [WorkflowHub](https://workflowhub.eu/), Galaxy, and [OpenEBench](https://openebench.bsc.es/), and, most importantly, accessible training resources and infrastructure.

 This tutorial is a modification of a Galaxy Training Network tutorial [Somatic variant calling](https://training.galaxyproject.org/training-material/topics/variant-analysis/tutorials/somatic-variants/tutorial.html) tutorial to provide training on how to preprocess, identify and visualise hCNV regions using Control-FreeC tool using tumor/normal samples pairs.
...
>    *e.g.*, `#normal` for normal tissue datasets and
>    *e.g.*, `#tumor` for tumor dataset.
>
>
...
>    > ![fastqafter](../../images/Control_Freec_fastqc_per_base_sequence_quality_plot_after.png "Sequence quality per base generated by FastQC after end trimming.")
>    > ![GCafter](../../images/Control_Freec_fastqc_per_sequence_gc_content_plot_afrer.png "Sequence quality per Sequence GC content generated by FastQC after end trimming.")
>    >
>    > > ### {% icon solution %} Solution
>    > >
...
![Controlfreec](../../images/Control_Freec_output.png " Detected regions of copy number gain/loss and LOH regions")
...
Import the DED file for the reagions from [Zenodo](https://doi.org/10.5281/zenodo.5697358):
>
>    ```
>    https://zenodo.org/record/5697358/files/Galaxy131-%5Bcapture_targets_chr5_12_17.bed%5D%20%281%29.bed?download=1.
>    ```

> ### {% icon hands_on %} Hands-on: Detection of copy-number changes
 > 1. Run {% tool [Control-FREEC](toolshed.g2.bx.psu.edu/repos/iuc/control_freec/control_freec/11.6+galaxy1) %}  with the following parameters
...
> ![Cicros](../../images/Control_Freec-%5BCircos_Plot%5D.png "Visualise the hCNV regions detected by Control_FREEC using Cicros")
