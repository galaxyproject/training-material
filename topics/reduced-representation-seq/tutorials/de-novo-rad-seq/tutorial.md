...
>
>    If you are not inspired, you can name it "STACKS RAD: population genomics without reference genome" for example...
>
...
> 3. Upload remaining training data from Zenodo or the data libraries (ask your instructor)
...
>
> 1. Use the `Charts` functionality through the Visualize button reachable on the `Radtags logs` file you just generated.
>
...
> 1. **Stacks: De novo map** {% icon tool %}: Run **Stacks** selecting the population usage. Specify each individual as a sample, a population map and a minimum depth of coverage of 3.
>
...
>    {: .comment}
>
> 2. Investigate the output files: `result.log` and `catalog.` (snps, alleles and tags).
>
...
>
> 1. **Stacks: populations** {% icon tool %}: Run the last step of **Stacks: De novo map** pipeline specifying data filtering options (minimum percentage of individuals in a population required to process a locus for that population: 0.75 , output options (VCF and Structure) and enabling SNP and haplotype-based F statistics calculation.
>
