...
title: Montreal Forced Aligner and new-fave
level: Introductory
zenodo_link: https://zenodo.org/records/21371207
questions:
  - How can you use Montreal Forced Aligner to align an audio and a TextGrid file and then extract the vowel formants in Galaxy?
objectives:
- Use the Montreal Forced Aligner to break down speech into its smallest sounds and align them with their corresponding transcription. The output can then be used to automatically extract the vowel measurements for analysis using new-fave.
time_estimation: 1H
contributions:
  authorship:
    - wallajess
...
This tutorial explains how to use the Montreal Forced Aligner (MFA) {% cite mcauliffe17_interspeech %} and new-fave {% cite fruehwald-maier-2026 %} on the Galaxy platform.
...
which can be selected in Galaxy. MFA outputs an aligned TextGrid, which is then used for the input of new-fave along with the original audio file. New-fave outputs multiple file options that include point measurements, full formant tracks, and Discrete Cosine coefficients.

MFA requires an audio or video file and an orthographic transcription as a TextGrid file.
...
> <hands-on-title> Extract vowel formant measurements </hands-on-title>
>
> 1. {% tool [Vowel formant extraction](new_fave_extract) %} with the following parameters:
>    - {% icon param-file %} *"Audio input"*: `the_fox_and_the_grapes.wav` (output of **Unzip** {% icon tool %})
>    - {% icon param-file %} *"Aligned TextGrid, ouput from MFA Alignt"*: `unzipped` (output of **Unzip** {% icon tool %})
>
>
>    > <comment-title> Output options </comment-title>
>    >
>    > New-fave can save the extracted measurements in several formats. Each format represents the formant information in a different way:
