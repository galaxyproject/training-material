...
## Jupyterlab features
[Jupyterlab](https://jupyterlab.readthedocs.io/en/stable/) notebook has been augmented with several useful features and together they make it ready-to-use for quick prototyping of AI projects. Being able to **serve online** makes it convenient to share it with other researchers and users. 

#### Features (as Python packages)
Accessibility to many features has been made possible by adding several compatible packages such as Bioblend for interacting with Galaxy, CUDA for GPU support, Scikit-learn and Tensorflow for creating AI algorithms and so on. Using Bioblend, the notebook can be **connected to Galaxy** and its histories, tools and workflows can be accessed directly in the notebook. They can easily be operated via commands from the notebook. GPUs have accelerated AI research, especially deep learning. Therefore, the backend of the Jupyterlab is **powered by GPU** to make long-running AI training programs finish faster by parallelizing. In addition, a standard model format, **open neural network exchange (ONNX)**, has been supported in the notebook to transform scikit-learn and tensorflow models to "onnx" files. These model files can be conveniently shared and used for inference. Galaxy also supports "onnx" file format which makes it easier to interoperate such models from a notebook to Galaxy and vice-versa.  matrix multiplications. Galaxy tool for remote job processing can also run on GPU. There are several additional packages suited for performing machine learning tasks such as [Open-CV](https://github.com/opencv/opencv) and [Scikit-Image](https://scikit-image.org/) for image processing, [NiBabel](https://nipy.org/nibabel/) package for processing images files. These packages are available to researchers and users in this notebook. Many other features such as GPU utilization dashboards for monitoring the GPU usage and system memory utilization, Voila for rendering output cells of a notebook in a separate tab hiding all code cells, interactive bqplots are integrated into the Jupyterlab notebook for researchers and users to manage their projects more efficiently.

#### Features (as plugins)
Features such as Git version control, ONNX AI models, Elyra AI pipelines, Voila rendering, have been added as Jupyterlab plugins. **Git version control** makes it easy to clone, commit and maintain codebase directly in the notebook. For example, repositories from Github can be easily cloned, updated and maintained.  Many notebooks are created serving different purposes. These notebooks can be knit together to form one pipeline where each notebook transforms data taking a different form of data from its previous notebook and passing on the transformed data to its next notebook. An example workflow created using **[Elyra AI](https://elyra.readthedocs.io/en/stable/)** can be found by name "METABRIC\_ML.pipeline" in the "elyra" folder of the opened Jupyterlab instance. These pipelines can also be executed remotely on different clusters too using its "runtimes" features that pull different docker containers. An example pipeline created in the notebook can be seen in the picture below.
