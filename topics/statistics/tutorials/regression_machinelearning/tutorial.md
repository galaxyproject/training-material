...
- Using regression, real-valued targets are learned using the training set and predicted
  using the test set.
- For each regression algorithm, it parameters should be optimised based on the dataset
time_estimation: 2H
contributors:
- khanteymoori
- anuprulez
...
In this tutorial you will learn how to use galaxy tools in [regression](https://en.wikipedia.org/wiki/Regression_analysis) problems. First, we will introduce regression briefly, and then examine linear regression, which models the relationship between a target variable and some explanatory variables or independet variables. Next, we will discuss gradiant boosting regressor, an advanced regressor model compared to simple linear regression that can model nonlinear relationship between variables. Then, we will show how to visulaize the reults in each step. 
...
In this tutorial we will build a regression models to build chronological age prediction based on DNA methylation. In the study [Jana Naue et al. 2017](https://www.sciencedirect.com/science/article/pii/S1872497317301643?via%3Dihub), biomarkers are examined to predict the chronological age of humans by analysing the DNA methylation pattern respectively. Different machine learning algorithms are used in this study to  make age prediction.
Over the years it is recognized that within each individual, [DNA methylation](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3174260/) changes with age. This knowledge is used to select useful biomarkers from DNA methylation dataset. The [CpGs sites](https://en.wikipedia.org/wiki/CpG_site) with the highest correlation to age are selected as the biomarkers(features). In this tutorial, specific biomarkers are analysed by machine learning algorithms to create an age prediction model.
...
Regression is a model that attemps to determine the relationship between one target variable and a series of independent variables. A regressor learns the mapping between the features of a dataset row and its target value. Inherently, it tries to fit a curve for the targets. This curve can be linear or non-linear. When the targets in a dataset are real numbers, the machine learning task becomes regression and each sample in the dataset has a real-valued output or target. Figure [1](#figure-1) shows how a (regression) curve is fit which explains most of the data points. Here, the curve is a straight line. The regression task is to learn this curve which explains the underlying distribution of the data points. The target for a new sample will lie on the curve learned by the regression task. These data points are the targets of the training dataset. A curve is learned which best explains all these points. The target for a new sample will lie along the curve learned by the regression task.

![regression](images/regression.png "Regression fit through data points.")

Linear regression is a technique used to analyse a linear relationship between input variables (independent variables) and a single target variable. A linear relationship means that the data points tend to follow a straight line. Simple linear regression involves only a single input variable. Figure 2 shows a data set with a linear relationship.

![regression](images/linear_regression_generated_data.png "Data points with the linear relationship.")

In linear regression, our goal is to find the line that best models the path of the data points. Figure 3 shows the data set we use in Figure 2 with a line of best fit through it. Linear regression helps us pick appropriate values for the fitting line coefficients. In this example we have only one input variable and the problem is simple linear regression. Note that in real problems we have more than one input variable. In this case, we call it multiple linear regression. Adding extra input variables just means that we’ll need to find more weights. 
...
As a benchmark, we will use the [DNA methylation dataset](https://www.sciencedirect.com/science/article/pii/S1872497317301643?via%3Dihub) to predict chronological age. One important reason to choose this dataset for age prediction task is that DNA methylation changes with age and this change occurs at specific CpG sites in humans. 
Over the years it is recognized that DNA methylation analysis which mostly occurs in a CpG sequence context can give additional information beside the DNA profile.  It has been revealed that DNA methylation alters with age within each individual. This alteration in DNA methylation has been shown to occur at specific CpG sites in all individuals however with individual differences in “speed”, showing more DNA methylation differences in aged twins compared to young ones.
Epigenomic and phenotypic changes which are age-dependent are also contained in these cells. Within each individual, DNA methylation changes with age. This knowledge is used to select useful biomarkers from DNA methylation dataset. The CpGs sites with the highest correlation to age are selected as the biomarkers (features). In this study, specific biomarkers are analysed by machine learning algorithms to create an age prediction model.
...
At the first step, to learn the mapping between several features and the targets, we will apply linear regressor. It learns features from training dataset and maps all the rows to their respective targets. The process of mapping gives a trained model. 
...
> >
> > In the linear regressor the coefficients of the best fitted line to data, is learned by the regressor.
> > 
> {: .solution}
>
{: .question}
...
After learning on the training dataset, we should evaluate the performance on the test dataset to know whether the learning algorithm learned a good model from the training dataset or not. This model is used to predict a new sample and a similar accuracy is expected. 
Now, we will predict age in the test dataset using this model in order to see if the model has learned important features which can generalise on a new dataset. The test dataset (`test_rows`) contains the same number of features but does not contain the `Age` column. This is predicted using the trained model.
...
Now we visualize and analyse the predictions using "Plot actual vs predicted curves and residual plots" tool in galaxy.
...
To learn the mapping between several features and the targets, in the next step, we will apply a regressor which is called
...
> [*Ensemble*](https://scikit-learn.org/stable/modules/ensemble.html#ensemble) method uses multiple learning models internally for better predictions and boosting is a method of converting weak learners into strong learners.
{: .comment}
...
>             - {% icon param-select %} *"Choose how to select data by column"*: `Select columns by column header name(s)`
>                - {% icon param-text %} *"Select target column(s)"*: `Age`
> 2. Rename the generated file to `gradiant_boosting_model`
{: .hands_on}

> ### {% icon question %} Question
>
> What is learned by the gradiant boosting regressor?
>
> > ### {% icon solution %} Solution
> >
...
>    - *"Select a Classification Task"*: `Load a model and predict`
>       - {% icon param-file %} *"Models"*: `gradiant_boosting_model`
>       - {% icon param-file %} *"Data (tabular)"*: `train_rows_test.csv`
>       - {% icon param-check %} *"Does the dataset contain header"*: `Yes`
>       - {% icon param-select %} *"Select the type of prediction"*: `Predict class labels`
> 2. Rename the generated file to `predicted_data_gradiant_boosting`
{: .hands_on}

Now we visualize and analyse the predictions using "Plot actual vs predicted curves and residual plots" tool.
> ### {% icon hands_on %} Hands-on: Check and visualize the predictions
> 1. **Plot actual vs predicted curves and residual plots** {% icon tool %} with the following parameters to visualise the predictions:
>    - {% icon param-file %} *"Select input data file"*: `test_rows_labels.csv`
>    - {% icon param-file %} *"Select predicted data file"*: `predicted_data_gradiant_boosting`
{: .hands_on}


 ![scatter_plot](images/true_vs_pred_scatter_ensemble.png "Scatter plot for true vs. predicted targets using gradiant boosting regressor.")

R2 score is 0.93 same as linear model but the RMSE is smaller (3.85) which also says that the predicted targets are closer to the true targets.

## Create data processing pipeline

At the final step, we will create a pipeline with **Pipeline builder** tool but this time, we just specify the regressor. The **Pipeline builder** tool will wrap this regressor and return a zipped file. We will use this zipped file with **Estimator attributes** tool set the search space of hyperparameters.
...
>                    - *"Choose a parameter name (with current value)"*: `gradiantboostingregressor_n_estimators: 100`
...
We can see in the scatter plot (figure [13](#figure-13)) that most of the points lie along the x=y curve. It means that the true and predicted ages are close to each other. The root mean square error (`RMSE`) is `3.74` and the R2 score is `0.94`.
...
As you can see in the figure, there is not any visible pattern with the plotted points and it means our model is good.
...
Moreover, we visualized the results using multiple plots to ascertain the robustness of machine learning tasks. There are many other regressors in the machine learning suite which can be tried out on these datasets to find how they perform. Different datasets can also be analysed using these regressors. The regressors have lots of parameters which can be altered while performing the analyses to see if they affect the prediction accuracy. It may be beneficial to perform hyperparameter search to tune these parameters of  regressors for different datasets. 
