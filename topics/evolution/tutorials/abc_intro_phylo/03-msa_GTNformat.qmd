...
.center[Multiple Sequence Alignment]
.center[Prof. Michael Charleston, PhD]
![Australian BioCommons logo](images/Australian-Biocommons-Logo-Horizontal-RGB.png)
![University of Tasmania logo](images/UTas logo.svg)
---
...
One of the best understood and best solved bioinformatics problem is *how to align two sequences*.
...
Given two sequences like ![DNA sequence GGCTGAA. Each base is a different colour G = orange, C = Blue, T = pink, A = green](images\DNA-GGGACTG.pdf-1.png) and ![DNA sequence GGGACTG. Each base is a different colour G = orange, C = Blue, T = pink, A = green](images\DNA-GGCTGAA.pdf-1.png), "an alignment" is a _mapping_ of their positions (a.k.a. "sites") to a common ordering, by inserting gaps in one sequence or another:
```
...
Our goal with sequence alignment is to identify which regions, down to individual positions in a molecular sequence, are _homologous_: that is, their shared evolutionary history is the same as that of the taxa of interest.
...
![Five unaligned DNA sequences are arranged vertically. Nucleotides are colour coded G = orange, C = blue, A = green, T = pink](images/03-msa-03-a-unaligned.pdf-1.png)
---
# A good alignment

This alignment reflects truth

![A possible alignment of five DNA sequences arranged vertically. Gaps have been introduced to align homologous sites. Nucleotides are colour coded G = orange, C = blue, A = green, T = pink](images/03-msa-03-b-truth.pdf-1.png)
---
# Although...

         
But alignment also looks ``good'' (?)

![A possible alignment of five DNA sequences arranged vertically. Gaps have been introduced to align homologous sites. Nucleotides are colour coded G = orange, C = blue, A = green, T = pink](images/03-msa-03-c-good1.pdf-1.png)
---
# Although...
...
![A possible alignment of five DNA sequences arranged vertically. Gaps have been introduced to align homologous sites. Nucleotides are colour coded G = orange, C = blue, A = green, T = pink
](images/03-msa-03-d-good2.pdf-1.png)
---
# This is nonsensical

Only matches and indels! Win?

![A possible alignment of five DNA sequences arranged vertically. Gaps have been introduced to align homologous sites. Nucleotides are colour coded G = orange, C = blue, A = green, T = pink](images/03-msa-03-e-nonsense.pdf-1.png)
...
This can be a bit of an art: you need to
...
# 
.center[Pair-wise Alignment]
.center[: Dynamic Programming Approach]
---
# Sequence Similarity

To do any kind of comparison we need a distance or similarity measure: without it we can't say whether, e.g., *these* two species are more similar than *those* two.
...
![An alignment of two DNA sequences. Positions that are not homologous are indicated by an asterisk and assigned the value ‘1’.](images/03-msa-05-hamming-distance.pdf-1.png)
...
```{=tex}
\begin{enumerate}
    \item  Linear cost: $$c = -dg$$ \\
            where $$d$$ is the _gap open penalty_;

    \item  Affine cost: $$c = -d - (g-1)e$$
...
More complex models prohibit the use of dynamic programming to solve the alignment (and must use \underline{heuristics}).
---
...
For two sequences of length $$x$$ and $$y$$, there are
$$\frac{(x+y)!}{x!\ y!}$$
possible alignments.

\medskip

For three sequences, of length $$x$$, $$y$$ and $$z$$ say, there are
$$\frac{(x+y+z)!}{x!\ y!\ z!}$$
alignments.
    
\end{minipage}\wad
```
\\begin{minipage}\[t\]{0.45\textwidth} For $n$ sequences of length 10, this increases rapidly: \\begin{center}
...
\begin{tabular}{|c|c|}
            ...
            \end{tabular}
```
\\end{center\\end{minipage} \medskip
...
Dynamic Programming (DP) is a common method to solve many types of problems, including pairwise sequence alignment.
...
---
#DP overview (cont.)
![Schematic representation of how alignment of two DNA sequences progresses. Three scenarios are shown: i) both sequences are identical ii) the sequences differ and a gap is introduced in the top sequence ii) the sequences differ and a gap is introduced in the bottom sequence.](images/03-msa-10-advancing-seqs.pdf-1.png)
...
2. Fill in the cells of the matrix from top-left to bottom right.
  ...
  2.1  Find the maximum score from the previous cells including 
        gap costs;
  2.2 Put the result into this new cell;
  2.3  _Note_ which cell we chose with a pointer or
        reference.
...
\begin{columns}
    ...
    \includegraphics[scale=0.5]{DPcell.pdf} \\
    \column[c]{0.576\linewidth}\raggedright
    We add the cost of the best solution to the previous cells (above, 
    to the left and above-left) to the best possible score for this
    cell.
\end{columns}
```
---
...
\begin{tabular}{c|ccccccccccc}
        $$s_{1}$$ & \texttt{H} & \texttt{E} & \texttt{A} & \texttt{G} &
        \texttt{A} & \texttt{W} & \texttt{G} & \texttt{H} & \texttt{E} &
        \texttt{-} & \texttt{E}  \\
        $$s_{2}$$ & \texttt{-} & \texttt{-} & \texttt{P} & \texttt{-} &
        \texttt{A} & \texttt{W} & \texttt{-} & \texttt{H} & \texttt{E} &
        \texttt{A} & \texttt{E}  \\
        \hline
...
-   Pairwise Sequence Alignment takes an amount of *time* that is proportional to the number of cells in the table, which is roughly the product of the lengths of the two sequences.
    ...
    That means it is \bigO{nm} if the lengths of the sequences are $$n$$ and $$m$$; usually as these are about the same we can write \bigO{n^{2}}: it's *quadratic in sequence length*.
...
-   With more sequences, the size of the table increase: for $$k$$ sequences the algorithm is \bigO{n^{k}}: not practical.
