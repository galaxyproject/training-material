...
>
> 1. Create two new histories for this tutorial one for every sampling time; name them `Barcode10`and `Barcode11`
>
>    {% snippet faqs/galaxy/histories_create_new.md %}
>
> 2. Import the samples Fastq.qz files via link from [Zenodo]({{ page.zenodo_link }}) or Galaxy shared data libraries to each history respectively:
>
...
>
> 1. Create a new history for for this part of the tutorial and name it `Pathogen Tracking among all samples`, where we will create collections from the results of the two samples' histories we had that are needed to track and visualize our found pathogens.
>
>    {% snippet faqs/galaxy/histories_create_new.md %}
>
> 2. Copy the following datasets from `Barcode10` history to the new created history, then copy the same files from `Barcode11` history to the new history too.
>
>     a. VFDB Accession
