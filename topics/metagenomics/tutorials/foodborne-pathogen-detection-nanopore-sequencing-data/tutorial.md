...
questions:
    - What are the steps to quality improve direct Nanopore datasets and prepare them for further analysis and investigations?
    - How to identify pathogens using sequencing data?
    - How to track the found pathogens through all your samples datasets?
objectives:
    - Check samples' sequences quality reports generated by FastQC and NanoPlot for metagenomics Nanopore data
    - Pre-process the sequencing data by quality improving and host filtering
    - Perform taxonomy profiling indicating and visualizing up to species level in the samples
    - Identify pathogens based on the found virulance factor gene products via assembly, identify strains and indicate all antimicrobial resistance genes in samples
...
WGS is more than a surveillance tool and was recommended by the European Centre for Disease Prevention and Control (ECDC) and the European Food Safety Authority (EFSA) for surveillance and outbreak investigation. WGS still requires isolation of the targeted pathogen, which is a time-consuming, not always straightforward, or successful process. **Sequencing methods without prior isolation could solve this issue**.
...
We will run all these steps using a single sub-workflow, then discuss each step and the results in more detail.
...
>    - {% icon param-file %} *"1: Nanopore reads of a sample"*: Choose Multiple Datasets and select both files; `Barcode10_Spike2.fastq.gz` and `Barcode11_Spike2b.fastq.gz`
...
Please do the same steps for both tags; `Barcode10`and `Barcode11`, or choose Multiple Datasets when running any tool and select both data files with respect to the step.
...
>            - {% icon param-file %} *"Data input files"*: `Barcode10Spike2` and `Barcode11Spike2b`  (Multiple Input dataset)
>
>
> 2. {% tool [FastQC](toolshed.g2.bx.psu.edu/repos/devteam/fastqc/fastqc/0.73+galaxy0) %} with the following parameters:
>    - {% icon param-file %} *"Raw read data from your current history"*: `Barcode10Spike2` and `Barcode11Spike2b`  (Multiple Input dataset)
>
> 3. {% tool [Porechop](toolshed.g2.bx.psu.edu/repos/iuc/porechop/porechop/0.2.4) %} with the following parameters:
>    - {% icon param-file %} *"Input FASTA/FASTQ"*: `Barcode10Spike2` and `Barcode11Spike2b`  (Multiple Input dataset)
>
...
>        - {% icon param-file %} *"Input 1"*: `outfile` (output of **Porechop** {% icon tool %}, Multiple Input dataset, the files from each tag) 
>
> 5. {% tool [NanoPlot](toolshed.g2.bx.psu.edu/repos/iuc/nanoplot/nanoplot/1.28.2+galaxy1) %} with the following parameters:
>    - *"Select multifile mode"*: `batch`
>        - *"Type of the file(s) to work on"*: `fastq`
>            - {% icon param-file %} *"Data input files"*: `out1` (output of **fastp** {% icon tool %}, Multiple Input dataset the files from each tag)
>
> 6. {% tool [FastQC](toolshed.g2.bx.psu.edu/repos/devteam/fastqc/fastqc/0.73+galaxy0) %} with the following parameters:
>    - {% icon param-file %} *"Raw read data from your current history"*: `out1` (output of **fastp** {% icon tool %}, Multiple Input dataset the files from each tag)
>
...
>                        - {% icon param-file %} *"FastQC output"*: `text_file` (output of **FastQC** {% icon tool %}, Multiple Input dataset the files from each tag)
{: .hands_on}
...
>
> Inspect the webpage output from **MultiQC** for `Barcode10` Tag
>
...
Please do the same steps for both tags; `Barcode10`and `Barcode11`, or choose Multiple Input Datasets when running any tool and select both data files with respect to the step.
...
>
> 2. {% tool [Filter Tabular](toolshed.g2.bx.psu.edu/repos/iuc/filter_tabular/filter_tabular/2.0.0) %} with the following parameters:
...
>
> 3. {% tool [Filter sequences by ID](toolshed.g2.bx.psu.edu/repos/peterjc/seq_filter_by_id/seq_filter_by_id/0.2.7) %} with the following parameters:
...
>    - *"Send results to a new history"*: `No`
>    - {% icon param-file %} *"Nanopore preprocessed reads"*: `Nanopore processed sequenced reads` the output from **Filter Sequence By ID** from the **Pre-Processing** sub-workflow - Choose Multiple Datasets, and select both files with the same name and different tags; `Barcode10` and `Barcode11`
...
Please do the same steps for both tags; `Barcode10`and `Barcode11`, or choose Multiple Input Datasets when running any tool and choose both data files with the same name and different tags with respect to the step.
...
> <comment-title></comment-title>
> While the steps is running you can move on to the next section **Gene based pathogenic identification** and run the steps there as well, both analyses can work in parallel.
{: .comment}
...
Now lets try the **Phinch visulization** tool running for `Barcode11` tag
...
We will run all these steps using a single sub-workflow, then discuss each step and the results in more detail. Please do the same steps for both tags; `Barcode10`and `Barcode11`.
...
>    - *"Send results to a new history"*: `No`
>    - {% icon param-file %} *"Nanopore Preprocessed reads"*: `Nanopore processed sequenced reads` with `Barcode10` tag, the output from **Filter Sequence By ID** {% icon tool %} from the **Pre-Processing** sub-workflow
>    - *"Sample ID"*: `Spike2Barcode10`
>    - {% icon param-file %} *"MLST Report Header"*: `MLST Report with Header` file you have just uploaded to your history
>
>    {% snippet faqs/galaxy/workflows_run.md %}
>
> 3. Repeat step 2 and run the same **sub-workflow** again for the second tag
>    - *"Send results to a new history"*: `No`
>    - {% icon param-file %} *"Nanopore Preprocessed reads"*: `Nanopore processed sequenced reads` with `Barcode11` tag the output from **Filter Sequence By ID** {% icon tool %} from the **Pre-Processing** sub-workflow
>    - *"Sample ID"*: `Spike2bBarcode11`
>    - {% icon param-file %} *"MLST Report Header"*: `MLST Report with Header` file you have just uploaded to your history
>
>    {% snippet faqs/galaxy/workflows_run.md %}
>
...
Please do the same steps for both tags; `Barcode10`and `Barcode11`, or choose Multiple Input Datasets when running the tool and choose both data files with respect to the step.
...
>                - *"Enter text that should be part of the computed value"*: `Spike2Barcode10` or `Spike2bBarcode11` respectively to the tag chosen for the first parameter (a sample id)
>        - {% icon param-repeat %} *"Insert components"*
>            - *"Choose the type of parameter for this field"*: `Text Parameter`
...
>
> 5. {% tool [FASTA-to-Tabular](toolshed.g2.bx.psu.edu/repos/devteam/fasta_to_tabular/fasta2tab/1.1.1) %} with the following parameters:
...
>                - *"in column"*: `c1`
>
> 7. Rename the output to `Sample Specific Contigs Tabular file`
>
...
>    - *"Sequence column"*: `c2`
>
> 9. Rename the output to `Sample Specific Contigs FASTA file`
{: .hands-on}
...
> <question-title></question-title>
>
> Inspect **Flye** and **Medaka consensus pipeline** output results from `Barcode10` tag
>
...
Please do the same steps for both tags; `Barcode10`and `Barcode11`, or choose Multiple Input Datasets when running any tool and choose both data files with respect to the step.
...
>
> 2. {% tool [Replace Text](toolshed.g2.bx.psu.edu/repos/bgruening/text_processing/tp_replace_in_column/1.1.3) %} with the following parameters:
>    - {% icon param-file %} *"File to process"*: `report` (output of **MLST** {% icon tool %})
>    - In *"Replacement"*:
>        - {% icon param-repeat %} *"Insert Replacement"*
>            - *"in column"*: `c1`
>            - *"Find pattern"*: `.* all contigs`
>            - *"Replace with"*: `Spike2Barcode10` or `Spike2bBarcode11` respectively to the tag chosen for the first parameter (the sample ID)
...
> <question-title></question-title>
>
> Inspect **MLST** {% icon tool %} results from `Barcode11` tag
>
...
> <question-title></question-title>
>
> Inspect **AMR Identified by NCBI** output files from `Barcode10`and `Barcode11` tags
>
...
>
> **Bacteria Pathogen**: A bacterial pathogen is usually defined as any bacterium that has the capacity to cause disease. Its ability to cause disease is called pathogenicity.
>
> **Virulence**: Virulence provides a quantitative measure of the pathogenicity or the likelihood of causing disease.
>
...
> 2. {% tool [Cut](Cut1) %} with the following parameters:
>    - *"Cut columns"*: `c13`
>    - {% icon param-file %} *"From"*: `report` (output of **ABRicate** {% icon tool %} run)
>
...
>                - *"in column"*: `c2`
>
> 4. Rename the output to `VFs ABRicate Sample Specific Formated Tabular`
>
> 5. {% tool [Add line to file](toolshed.g2.bx.psu.edu/repos/bgruening/add_line_to_file/add_line_to_file/0.1.0) %} with the following parameters:
>    - *"text to add"*: `Spike2Barcode10` or `Spike2bBarcode11` respectively to the tag (the sample ID)
>    - {% icon param-file %} *"input file"*: `out_file1` (output of the second **Cut** {% icon tool %} from step 10)
>
> 6. Rename the output `VFDB Accession Tabular with SampleID as a header`
{: .hands-on}
...
> <comment-title>Data upload</comment-title>
>
> In this section, we will run steps that can run in parallel to **Taxonomy Profiling**, **Gene based pathogenic identification** and **Pathogen Tracking among all samples** steps, so feel free to run it in the time you are waiting for the other steps to finish running.
{: .comment}
...
2. Then we indetify variants and single nucleotide variants using [__Clair3__](https://hpc.nih.gov/apps/Clair3.html), which is designed specifically for Nanopore datasets giving better results than other variant calling tools as well as its new and up-to-date.

    [__Medaka consensus tool__ and __medaka variant tool__](https://github.com/nanoporetech/medaka) can be also used instead of **Clair3**, they give similar results but they are much slower then **Clair3** and with much fewer tools' options. So we recommend **Clair3** that we used in this sub-workflow.
...
> <question-title></question-title>
>
> Now let's inspect the sub-workflow outputs together for `Barcode10` tag
>
...
> <question-title></question-title>
>
> Inspect **bcftools consensus** `Barcode11` tag
>
...
> <comment-title></comment-title>
>
> If you did not get your **Gene based pathogenic identification** section output files needed yet or you got an error with any, you can go on and upload them all or the ones missing from Zenodo so you can start this workflow, please don't forget to create the collections for them as explained in the pervious hands-on.
>
> > <hands-on-title>Optional Data upload</hands-on-title>
> >
...
>
> 1. Create a collection named `VFDB Accession Column` with `VFDB Accession` files of both tags; `Barcode10` and `Barcode11`
>
>    {% snippet faqs/galaxy/collections_build_list.md %}
>
> 2. Create a collection named with `List of All Samples VFDB Accession Column with Sample ID Tabulars` with `VFDB Accession Tabular with SampleID as a header` files
>
> 3. Create a collection named `All Samples ABRicate Tabulars` with `VFs ABRicate Sample Specific Formatted Tabular` files
>
> 4. Create a collection named `All Samples Contigs Fasta files` with `Sample Specific Contigs FASTA file` files
>
{: .hands_on}
...
>    - *"text to add"*: `All_VFs`
>    - {% icon param-file %} *"input file"*: `output` (output of 1st **Collapse Collection** {% icon tool %})
>
> 5. {% tool [Multi-Join](toolshed.g2.bx.psu.edu/repos/bgruening/text_processing/tp_multijoin_tool/1.1.1) %} with the following parameters:
...
>        - *"List of Fields"*: `c1`
>
> 8. {% tool [Replace](toolshed.g2.bx.psu.edu/repos/bgruening/text_processing/tp_find_and_replace/1.1.4) %} with the following parameters:
>    - {% icon param-file %} *"File to process"*: `outfile` (output of 1st **Replace** {% icon tool %})
>    - In *"Find and Replace"*:
>        - {% icon param-repeat %} *"Insert Find and Replace"*
>            - *"Find pattern"*: `Acc_0`
>            - *"Replace with"*: `0`
...
>            - *"Find and Replace text in"*: `entire line`
>
> 9. {% tool [Replace](toolshed.g2.bx.psu.edu/repos/bgruening/text_processing/tp_find_and_replace/1.1.4) %} with the following parameters:
>    - {% icon param-file %} *"File to process"*: `outfile` (output of 2nd **Replace** {% icon tool %})
>    - In *"Find and Replace"*:
>        - {% icon param-repeat %} *"Insert Find and Replace"*
>            - *"Find pattern"*: `Acc_\S*`
>            - *"Replace with"*: `1`
...
>            - *"Find and Replace text in"*: `entire line`
>
> 10. {% tool [Advanced Cut](toolshed.g2.bx.psu.edu/repos/bgruening/text_processing/tp_cut_tool/1.1.0) %} with the following parameters:
>    - {% icon param-file %} *"File to cut"*: `outfile` (output of 3rd **Replace** {% icon tool %})
>    - *"Operation"*: `Discard`
>    - *"Cut by"*: `fields`
>        - *"List of Fields"*: `c1,2`
>
> 11. {% tool [Paste](Paste1) %} with the following parameters:
>    - {% icon param-file %} *"Paste"*: `output` (output of 1st **Advanced Cut** {% icon tool %})
>    - {% icon param-file %} *"and"*: `output` (output of 2nd **Advanced Cut** {% icon tool %})
>
> 12. {% tool [Transpose](toolshed.g2.bx.psu.edu/repos/iuc/datamash_transpose/datamash_transpose/1.1.0+galaxy2) %} with the following parameters:
>    - {% icon param-file %} *"Input tabular dataset"*: `out_file1` (output of **Paste** {% icon tool %})
>
> 13. {% tool [Heatmap w ggplot](toolshed.g2.bx.psu.edu/repos/iuc/ggplot2_heatmap/ggplot2_heatmap/3.3.5+galaxy1) %} with the following parameters:
...
>    - {% icon param-file %} *"File to split"*: `output` (output of 2nd **Collapse Collection** {% icon tool %})
>    - *"on column"*: `c13`
>    - *"Include header in splits?"*: `Yes`
>
> 2. {% tool [Cut](Cut1) %} with the following parameters:
>    - *"Cut columns"*: `c2,c3,c4`
>    - {% icon param-file %} *"From"*: `output` (output of 2nd **Collapse Collection** {% icon tool %})
>
> 4. {% tool [Filter sequences by ID](toolshed.g2.bx.psu.edu/repos/peterjc/seq_filter_by_id/seq_filter_by_id/0.2.7) %} with the following parameters:
>    - {% icon param-file %} *"Sequence file to be filtered"*: `output` (output of 1st **Collapse Collection** {% icon tool %})
>    - *"Filter using the ID list from"*: `tabular file`
...
>
> 4. {% tool [bedtools getfasta](toolshed.g2.bx.psu.edu/repos/iuc/bedtools/bedtools_getfastabed/2.30.0+galaxy1) %} with the following parameters:
...
> 5. {% tool [ClustalW](toolshed.g2.bx.psu.edu/repos/devteam/clustalw/clustalw/2.1+galaxy1) %} with the following parameters:
...
> 6. {% tool [FASTTREE](toolshed.g2.bx.psu.edu/repos/iuc/fasttree/fasttree/2.1.10+galaxy1) %} with the following parameters:
...
> 7. {% tool [Newick Display](toolshed.g2.bx.psu.edu/repos/iuc/newick_utils/newick_display/1.6+galaxy1) %} with the following parameters:
