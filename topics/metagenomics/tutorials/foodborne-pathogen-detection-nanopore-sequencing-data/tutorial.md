...
- Read trimming and filtering with **Porechop** and **Fastp** based on the quality identified with **FastQC** and **Nanoplot**
...
- Re-quality check after read trimming and filtering with **FastQC** and **Nanoplot** and report aggregation with **MultiQC**
...
1. Assign reads to taxon using **Kraken2** and **Kalamari**, a database of completed assemblies for metagenomics-related tasks used widely in contamination and host filtering
...
1. Genome assembly to get contigs, i.e. longer sequences, using [__metaflye__](https://www.nature.com/articles/s41592-020-00971-x) then assembly polishing using [__medaka consensus pipeline__](https://github.com/nanoporetech/medaka) and visualizing the assembly graph using [__Bandage Image__](https://rrwick.github.io/Bandage/)
...
> > <solution-title></solution-title>
> >
> > 1. After **Flye** we have got 115 contigs
> > 2. After **Medaka consensus pipeline** all 115 contigs were kept, which means that the quality of the **Flye** run was high that the polishing did not remove any of the contigs.
...
> >    1. Tet(C), which resists [TETRACYCLINE](https://medlineplus.gov/druginfo/meds/a682098.html). It's found in contig 109 from the position 1634 till 2809, with 100% coverage, so 100% of gene is covered in this contig.
...
1. **Map reads to the reference genome** of the species of the pathogen we want to test our samples against using **Minimap2** as our datasets are Nanopore
...
2. **Identify variants and single nucleotide variants** using [__Clair3__](https://hpc.nih.gov/apps/Clair3.html), which is designed specifically for Nanopore datasets giving better results than other variant calling tools as well as its new and up-to-date.
...
3. **Left-align and normalize indels** using [__bcftools norm__](https://samtools.github.io/bcftools/bcftools.html)
...
4. **Filter variants** to keep only the pass and good quality variants using [__SnpSift Filter__](http://pcingola.github.io/SnpEff/)
...
>    - *"Send results to a new history"*: `No`
>    - {% icon param-collection %} *"All Samples Contigs Fasta files"*: `Contigs`
>    - {% icon param-collection %} *"All Samples ABRicate Tabulars"*: `VFs`
>    - {% icon param-collection %} *"List of All Samples VFDB Accession Column with Sample ID Tabulars"*: `VFs accessions with SampleID`
>    - {% icon param-collection %} *"VFDB Accession Column"*: `VFs accessions`
>
>    {% snippet faqs/galaxy/workflows_run.md %}
>
{: .hands_on}
