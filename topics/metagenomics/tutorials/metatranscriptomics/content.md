...
5 files and a collection are generated by **MetaPhlAn2** {% icon tool %}:
...
> ### {% icon hands_on %} Hands-on: Cut taxon relative abundances table
...
>
> 2. {% tool [Export to GraPhlAn]() %} with the following parameters:
...
>
> 3. {% tool [Generation, personalization and annotation of tree]() %} with the following parameters:
...
>
> 4. {% tool [GraPhlAn]() %} with the following parameters:
...
>
> 1. {% tool [HUMAnN]() %} with the following parameters:
...
2. A tabular file with the **pathways and their abundance**:
    ...
    > > 3. There are 116 lines in the pathway file, including the lines with species information. To compute the number of gene families, we need to apply a similar approach as for the gene families by removing the lines with `|` in them using the tool {% tool [Split a HUMAnN table]() %}.
...
>
> 1. {% tool [Renormalize a HUMAnN generated table]() %} with
...
> ### {% icon hands_on %} Hands-on: Normalize the gene family abundances
>
> 1. {% tool [Unpack pathway abundances to show genes included]() %} with
...
>
> 1. {% tool [Regroup HUMAnN table features]() %} with the following parameters:
...
>
> 1. {% tool [Combine MetaPhlAn2 and HUMAnN2 outputs]() %}  with the following parameters:
