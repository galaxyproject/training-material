...
- A predicted image (`.tiff`)
- A predicted tensor matrix (`.npy`)

Below is a visual **example** based on the output generated by the **🧬 NucleiSegmentationBoundaryModel**.  
This is a **composite image** created for illustration purposes by combining multiple views side by side.

The actual output is a **TIFF file**, and both predicted images reside in the same TIFF file.  
👉 **See tip below** for how to properly visualise the output locally.
