...
> 5. **Histogram equalization** {% icon tool %} with the following parameters to normalize the intensity values:
>    - {% icon param-file %} *"Source file"*: `input_segmented_labeled` file (output of **Binary 2 Label** {% icon tool %})
>    - *"Histogram Equalization Algorithm"*: `Streching`
> 6. Rename {% icon galaxy-pencil %} the generated file to `input_segmented_labeled_normalized`
> 7. **Convert image** {% icon tool %} with the following parameters to convert the image to PNG:
>    - {% icon param-file %} *"Input Image"*: `input_segmented_labeled_normalized` file (output of **Histogram equalization** {% icon tool %})
>    - *"Output data type"*: `PNG`
> 8. Rename {% icon galaxy-pencil %} the generated file to `viz_segmented`
> 9. **Overlay Segmentation Mask** {% icon tool %} with the following parameters to convert the image to PNG:
...
>    - *"Plot Labels"*: `yes`
>    - *"Label Color"*: `yellow`
> 10. Click on the {% icon galaxy-eye %} (eye) icon next to the file name, to look at the file content and assess the segmentation performance
> 11. **Count Objects** {% icon tool %} with the following parameters to count the segmented objects in the image:
>    - {% icon param-file %} *"Source file"*: `input_segmented_labeled` file (output of **Binary 2 Label** {% icon tool %})
>
>    > ### {% icon question %} Questions
>    >
>    > 1. What does **Binary 2 Label** {% icon tool %}?
>    > 2. Why do we have to use "**Histogram equalization** {% icon tool %}?
>    > 3. How many objects were segmented?
>    >
>    > > ### {% icon solution %} Solution
>    > > 1. The tool assigns each connected component (e.g., segmented cell) in the image an object id and stores it as intensity value.
>    > > 2. The object ids generated by **Binary 2 Label** {% icon tool %} are relatively low. To make them visible the values have to be stretched to a larger range of visible 
intensity values.
>    > > 3. The **Count Objects** {% icon tool %} counted XX objects.
>    > {: .solution }
>    {: .question}
{: .hands_on}
