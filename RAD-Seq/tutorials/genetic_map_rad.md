...
In the study of [Armores *et al.* 2011](http://www.genetics.org/content/188/4/799), a spotted gar *Lepisosteus oculatus* genetic map was developed and we will work here with data from a single linkage group. The gar genetic map is an F1 pseudotest cross between two parents and 94 of their F1 progeny. We took the markers that appeared in one of the linkage groups and worked backwards to provide the raw reads from all of the stacks contributing to that linkage group. the basic idea of how to assemble this data into a genetic map closely follows the [existing Stacks tutorial](http://catchenlab.life.illinois.edu/stacks/tut.php). However, this data is already cleaned and so you can proceed from part 2 of the tutorial and all the way through to building the linkage group in JoinMap.
